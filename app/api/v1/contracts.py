...
from app.model import Listing, BondToken, MembershipToken, CouponToken, JDRToken
...
# [JDR]公開中トークン一覧
# ------------------------------
class JDRContracts(BaseResource):
    """
    Handle for endpoint: /v1/JDR/Contracts
    """
    ...
    def on_get(self, req, res):
        LOG.info('v1.contracts.JDRContracts')

        session = req.context["session"]

        # Validation
        request_json = JDRContracts.validate(req)
        ...
        for i in reversed(range(0, cursor)):
            ...
            # token_templateがIbetDepositaryReceiptのものに対して詳細情報を取得する
            # NOTE: TokenListコントラクトに未登録のものについては読み飛ばす
            if token[1] == 'IbetDepositaryReceipt':
                token_detail = self.get_token_detail(
                    token_id=i,
                    company_list=company_list,
                    token_address=token[0],
    ...
    def get_token_detail(self, token_id, token_address, token_template,
        ...
        try:
            # IbetDepositaryReceiptトークンコントラクト設定
            TokenContract = Contract.get_contract(
                'IbetDepositaryReceipt',
                to_checksum_address(token_address)
            )

            # 取扱停止状態のトークンはリストに返さない
            if not TokenContract.functions.status().call():
                return None
            ...
            token.token_template = token_template
