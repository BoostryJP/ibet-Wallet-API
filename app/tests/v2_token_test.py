...
# [会員権]トークン一覧参照API
# /v2/Token/Membership
class TestV2TokenGetMembershipTokens():
    ...
    def test_membershiplist_normal_1(self, client, session, shared_contract):
        ...
        # TokenListコントラクト
        token_list = TestV2TokenGetMembershipTokens.tokenlist_contract()
        ...
        attribute = TestV2TokenGetMembershipTokens.token_attribute(exchange_address)
        token = membership_issue(issuer, attribute)
        membership_register_list(issuer, token, token_list)

        # 取扱トークンデータ挿入
        TestV2TokenGetMembershipTokens.list_token(session, token)
    ...
    def test_membershiplist_normal_2(self, client, session, shared_contract):
        ...
        exchange_address = \
            to_checksum_address(
                shared_contract['IbetMembershipExchange']['address'])
        for i in range(0, 2):
            attribute = TestV2TokenGetMembershipTokens.\
...
# [クーポン]トークン一覧参照API
# /v2/Token/Coupon
class TestV2TokenGetCouponTokens():
    ...
    def test_couponlist_normal_1(self, client, session, shared_contract):
        ...
        # TokenListコントラクト
        token_list = TestV2TokenGetCouponTokens.tokenlist_contract()
        config.TOKEN_LIST_CONTRACT_ADDRESS = token_list['address']
        ...
        attribute = TestV2TokenGetCouponTokens.token_attribute(exchange_address)
        token = issue_coupon_token(issuer, attribute)
        coupon_register_list(issuer, token, token_list)

        # 取扱トークンデータ挿入
        TestV2TokenGetCouponTokens.list_token(session, token)
    ...
    def test_couponlist_normal_2(self, client, session, shared_contract):
        ...
        exchange_address = \
            to_checksum_address(
                shared_contract['IbetCouponExchange']['address'])
        for i in range(0, 2):
            attribute = TestV2TokenGetCouponTokens.\
                token_attribute(exchange_address)
            token = issue_coupon_token(issuer, attribute)
            coupon_register_list(issuer, token, token_list)
            issued_list.append(token)
