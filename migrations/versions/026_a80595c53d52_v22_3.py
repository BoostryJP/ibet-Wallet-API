...
def upgrade():
    ...
    if op.get_context().dialect.name != "mysql":
        op.execute("CREATE SEQUENCE notification_id_seq START 1")
...
def downgrade():
    ...
    if op.get_context().dialect.name != "mysql":
        op.execute("DROP SEQUENCE notification_id_seq")
    # NOTE: Update sqlalchemy-migrate version
    tables = sa.inspect(connection).get_table_names()
    if "migrate_version" in tables:
        op.get_bind().execute(f"UPDATE migrate_version SET version = 0;")
    # ### end Alembic commands ###
