...
from app.model.schema.base.base import SortOrder
    ...
    async def fetch_sync_status(self):
        async with TCPConnector(limit=2, keepalive_timeout=0) as tcp_connector:
            async with ClientSession(
                ...
                try:
                    node_info: GetBlockSyncStatusResponse = (
                        await connector.get_node_info(session, self.base_url)
                    )
                except Exception as e:
                    if hasattr(self, "emit_no_wait"):
                        self.emit_no_wait(Error(e, self))
                    return
                self.update_current_block(node_info.latest_block_number)
                self.update_is_synced(node_info.is_synced)
                if (
                    self.tui.state.current_block_number is None
                    ...
                    query.from_block_number = max(
                        node_info.latest_block_number - self.tui.lot_size - 1, 0
                    )
