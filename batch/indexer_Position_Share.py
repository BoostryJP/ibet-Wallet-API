...
class Processor:
    ...
    def __get_idx_position_block_number(db_session: Session, token_address: str, exchange_address: str):
        ...
        _idx_position_block_number = db_session.query(IDXPositionShareBlockNumber).\
            ...
            filter(IDXPositionShareBlockNumber.exchange_address == exchange_address).first()
    ...
    def __set_idx_position_block_number(db_session: Session, target_token_list: TargetTokenList, block_number: int):
        ...
        for target_token in target_token_list:
            _idx_position_block_number = db_session.query(IDXPositionShareBlockNumber). \
                filter(IDXPositionShareBlockNumber.token_address == target_token.token_contract.address).\
                filter(IDXPositionShareBlockNumber.exchange_address == target_token.exchange_address).first()
            if _idx_position_block_number is None:
