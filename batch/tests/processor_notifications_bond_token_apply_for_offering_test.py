...
def watcher_apply_for_offering():
...
class TestProcessorNotificationsBondTokenWatchApplyForOffering:
    ...
    @staticmethod
    def bond_apply_for_offering(bond_exchange: dict, personal_info: dict, payment_gateway: dict):
    ...
    ####################################################################
    # Normal_1
    def test_normal_1(self, watcher_apply_for_offering, shared_contract):
        ...
        bond_token = self.bond_apply_for_offering(bond_exchange, personal_info, payment_gateway)
        ...
        # Normal_2 : データ無
    def test_normal_2(self, watcher_apply_for_offering):
        engine = create_engine(config.DATABASE_URL, echo=False)
        db_session = scoped_session(sessionmaker())
        db_session.configure(bind=engine)
