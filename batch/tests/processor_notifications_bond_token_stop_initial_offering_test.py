...
def watcher_stop_initial_offering():
...
class TestProcessorNotificationsBondTokenWatchStopInitialOffering:
    ...
    @staticmethod
    def bond_stop_initial_offering(bond_exchange, personal_info):
    ...
    ####################################################################
    # Normal_1
    def test_normal_1(self, watcher_stop_initial_offering, shared_contract):
        ...
        bond_token = self.bond_stop_initial_offering(
    ...
    def test_normal_2(self, watcher_stop_initial_offering):
        engine = create_engine(config.DATABASE_URL, echo=False)
        db_session = scoped_session(sessionmaker())
        db_session.configure(bind=engine)
        started_timestamp = datetime.datetime.utcnow().replace(microsecond=0)
