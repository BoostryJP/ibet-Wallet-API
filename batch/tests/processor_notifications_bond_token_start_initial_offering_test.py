...
def watcher_start_initial_offering(shared_contract, session):
...
class TestProcessorNotificationsBondTokenWatchStartInitialOffering:
    """
    ...
    def bond_start_initial_offering(bond_exchange, personal_info, token_list):
        ...
        #   4) 債券トークンの新規募集開始（initialOfferingStatus = True）
        attribute = {
            'name': 'テスト債券',
            'symbol': 'BOND',
            'totalSupply': 1000000,
    ...
    def test_normal_1(self, watcher_start_initial_offering, shared_contract, session):
        ...
        assert notification.args == {'status': True}
