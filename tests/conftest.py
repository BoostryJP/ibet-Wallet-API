...
def payment_gateway_contract() -> DeployedContract:
    ...
    contract_address, abi = Contract.deploy_contract(
        'PaymentGateway', [], deployer)

    contract = Contract.get_contract('PaymentGateway', contract_address)
    tx_hash = contract.functions.addAgent(agent).transact(
...
def db_engine(request):
    ...
    if worker_id is not None:
        test_engine = get_engine(config.DATABASE_URL + "_" + worker_id)
    else:
        test_engine = engine
    return test_engine
...
def db(request, db_engine):
    ...
    if db_engine.name != "mysql":
        ...
        Notification.notification_id_seq.create(bind=db_engine)

    # Create DB session
    db = sessionmaker(autocommit=False, autoflush=True, bind=db_engine)()
    ...
    company_list.engine = db_engine
    web3_utils.engine = db_engine
