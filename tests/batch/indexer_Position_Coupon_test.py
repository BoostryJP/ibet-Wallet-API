...
class TestProcessor:
    ...
    async def test_normal_8(
        ...
        _idx_position_coupon_block_numbers: Sequence[IDXPositionCouponBlockNumber] = (
            session.scalars(
        ...
        assert len(_idx_position_coupon_block_numbers) == 0
    ...
    async def test_normal_12(
        ...
        for i in range(1001):
            transfer_coupon_token(
                ...
                token,
                {
                    "account_address": to_checksum_address(f"0x{hex(i)[2:].zfill(40)}"),
                    "password": "password",
                },
                1,
            )
    ...
    async def test_error_3(
        ...
        with (
            mock.patch("batch.indexer_Position_Coupon.asyncio", asyncio_mock),
            mock.patch(
                "batch.indexer_Position_Coupon.Processor.initial_sync",
                return_value=True,
            ),
            mock.patch(
                "web3.AsyncWeb3.AsyncHTTPProvider.make_request",
