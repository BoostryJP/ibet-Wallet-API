...
class TestCompanyInfoCompanyInfo:
    ...
    # 正常系1-2： 発行会社リストに指定したアドレスの情報が存在(ローカルJSON) & null属性有り
    def test_normal_1_2(self, client: TestClient, mocked_company_list, session: Session):
    ...
    # 正常系1-3： 発行会社リストに指定したアドレスの情報が存在(DB)
