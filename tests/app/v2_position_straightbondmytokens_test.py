...
class TestV2StraightBondMyTokens:
    ...
    # 債券トークンの売注文中状態を作成
    @staticmethod
    def generate_bond_commitment(bond_exchange, personal_info, payment_gateway, token_list):
        issuer = eth_account['issuer']
        trader = eth_account['trader']
        agent = eth_account['agent']
        ...
        # ＜投資家オペレーション＞
        #   1) Make売
        offer_bond_token(trader, bond_exchange, bond_token, 50, 1001)
    ...
    #   →　決済代行：決済　→　投資家：Make売
    def test_position_normal_5_1(self, client, session, shared_contract):
        bond_exchange = shared_contract['IbetStraightBondExchange']
        personal_info = shared_contract['PersonalInfo']
        payment_gateway = shared_contract['PaymentGateway']
        token_list = shared_contract['TokenList']
        ...
        bond_token = TestV2StraightBondMyTokens.generate_bond_commitment(
            bond_exchange, personal_info, payment_gateway, token_list)
        token_address = bond_token['address']
    ...
    # 発行体：新規発行　→　発行体：募集（Make売）　→　投資家：Take買
    #   →　決済代行：決済　→　投資家：Make売
    def test_position_normal_5_2(self, client, session, shared_contract):
        bond_exchange = shared_contract['IbetStraightBondExchange']
        personal_info = shared_contract['PersonalInfo']
        payment_gateway = shared_contract['PaymentGateway']
        token_list = shared_contract['TokenList']
    ...
    def test_position_error_4(self, client, session):
        ...
        assert resp.json['meta'] == {
            ...
            'description': 'method: POST, url: /v2/Position/StraightBond'
        }
